...
In contrast, the goal of melody transcription is to output the \emph{notes} of the melody, where a note is defined by an onset time, a discrete \pl{seems like the key thing is the discreteness, so should emphasize that more?} pitch, and an offset time. 
\pl{also, don't you have to figure out the musical time (measures + beat within measure), not just the number of seconds?}
